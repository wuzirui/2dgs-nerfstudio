...
@dataclass
class SurfelSplatModelConfig(ModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: SurfelSplatModel)
...
class SurfelSplatModel(SplatfactoModel):
    """Nerfstudio's implementation of 2D Gaussian Splatting

    Args:
        config: configuration to instantiate model
    """

    config: SurfelSplatModelConfig
    ...
    def load_state_dict(self, dict, **kwargs):  # type: ignore
        ...
        self.scales = torch.nn.Parameter(torch.zeros(newp, 2, device=self.device))
